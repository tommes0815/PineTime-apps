...
static const char *_state2color[] = {
    [0] = GUI_COLOR_LBL_BASIC_RED,
    [1] = GUI_COLOR_LBL_BASIC_BLUE,
    [2] = GUI_COLOR_LBL_DARK_GREEN,
};
...
static void _home_time_set_bt_label(home_time_widget_t *htwidget)
{
    ...
    if (gatt_pan_connected())
        pan_st = 2;
    const char *color = _state2color[pan_st];

    lv_label_set_text_fmt(htwidget->lv_ble, "%s "LV_SYMBOL_BLUETOOTH"#", color);
}
